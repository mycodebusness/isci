...
import { Etudiant as Payment } from "@/lib/types";
...
import { deleteEtudiant } from "@/lib/actions";
...
export const columns: ColumnDef<Payment>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <div className="capitalize">{row.getValue("noms")}</div>,
  },
  {
    accessorKey: "sexe",
    header: "Genre",
  ...
  {
    accessorKey: "lieunaiss",
    header: "lieu naiss",
    cell: ({ row }) => (
      <div className="uppercase">{row.getValue("lieunaiss")}</div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">
        {new Date(row.original.datenaiss).toDateString()}
      </div>
    ),
  },
  {
  ...
  {
    accessorKey: "adressetudiant",
    header: "Adresse",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("adressetudiant")}</div>
    ),
  },
  {
    accessorKey: "email",
  ...
  {
    accessorKey: "telresponsable",
    header: "responsable",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("telresponsable")}</div>
    ),
  },
      ...
      return (
        <div className="flex gap-3">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  ...
                  href={`/dashboard/etudiant/ajouter`}
                >
          ...
          <Dialog>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="flex gap-2 justify-center items-center">
                  ...
                  <p>supprimer l&apos;étudiant {row.original.noms}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle> Suppression étudiant</DialogTitle>
                <DialogDescription>
                  Etes-vous sur de supprimer le client {row.original.noms}{" "}
                  définitivement?
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter className="justify-end">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      await deleteEtudiant(row.original.matricule);
                      toast({
                        title: "Supprimer",
    ...
    <div className="w-full px-4 pt-4">
      <div className="flex items-center pb-4">
        <Input
          ...
          value={(table.getColumn("noms")?.getFilterValue() as string) ?? ""}
          onChange={(event) =>
            table.getColumn("noms")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
      </div>
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          Total : {table.getFilteredRowModel().rows.length} étudiant(s).
        </div>
